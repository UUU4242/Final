...
class Route:
    ...
    def __init__(self, locations, requests, problem):
        ...
        if self.feasible:
            ...
            self.distance = 100000000  # 修复：防止数值爆炸导致 ALNS 失效；优化：一亿罚金倒逼合法
    ...
    def isFeasible(self):
        ...
        """
        if self.locations[0].nodeID != self.problem.depot.nodeID or self.locations[
            -1].nodeID != self.problem.depot.nodeID:
            # print(f"DEBUG: 违规 -> 首尾不是仓库")
            return False
        ...
        for i in range(1, len(self.locations) - 1):
            ...
            if int(curNode.typeLoc) == 1:
                curLoad += abs(curNode.demand)
            ...
            if curLoad > self.problem.capacity or curLoad < 0:
                # 🧹 已关闭：违规 -> 爆仓 打印
                return False
    ...
    def addRequest(self, request, preNode_index, afterNode_index):
        ...
        if afterInsertion.feasible:
            # 修复：必须在真实插入前计算 cost，否则会引发双重插入违规！
