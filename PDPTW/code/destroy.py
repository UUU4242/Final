...
class Destroy:
    ...
    '''Destroy method number 10: Shaw / Relatedness Removal '''

    # 核心思想：移除时空相关性最高的订单“簇”，为修复算子创造完美重组空间
    def executeShawRemoval(self, nRemove, randomGen):
        if len(self.solution.served) == 0:
            return

        # 1. 随机选一个已服务的订单作为“种子”
        seed_req = randomGen.choice(self.solution.served)
        removed_requests = [seed_req]

        # 权重参数配置 (alpha: 距离权重, beta: 时间窗权重, gamma: 载重权重)
        # PDPTW 中距离和时间最重要
        alpha = 1.0
        beta = 1.0
        gamma = 0.5
        ...
        # 2. 滚雪球式寻找最相关的订单
        while len(removed_requests) < nRemove and len(removed_requests) < len(self.solution.served):
            # 随机从已被选中的订单中挑一个作为参考基准
            ref_req = randomGen.choice(removed_requests)

            relatedness_list = []

            # 计算现存所有订单与参考订单的相关性 (Relatedness)
            for req in self.solution.served:
                if req in removed_requests:
                    continue

                # a. 地理距离差异 (取货点距离 + 送货点距离)
                dist = (self.problem.distMatrix[ref_req.pickUpLoc.nodeID][req.pickUpLoc.nodeID] +
                        self.problem.distMatrix[ref_req.deliveryLoc.nodeID][req.deliveryLoc.nodeID])

                # b. 时间窗差异 (取货时间窗 + 送货时间窗差异)
                time_diff = (abs(ref_req.pickUpLoc.startTW - req.pickUpLoc.startTW) +
                             abs(ref_req.deliveryLoc.startTW - req.deliveryLoc.startTW))

                # c. 载重需求差异
                demand_diff = abs(ref_req.pickUpLoc.demand - req.pickUpLoc.demand)

                # 综合相关性 R值 (R值越小，说明越相似、关联越紧密)
                r_val = alpha * dist + beta * time_diff + gamma * demand_diff
                relatedness_list.append((req, r_val))

            # 按 R值 从小到大排序 (最相关的排在前面)
            relatedness_list.sort(key=lambda x: x[1])

            # 3. 引入控制随机性 (避免每次拆解都一模一样卡死)
            # 公式: index = (random^p) * length, p=3 是经典设值
            rand_val = randomGen.random()
            idx = int((rand_val ** 3) * len(relatedness_list))

            removed_requests.append(relatedness_list[idx][0])
