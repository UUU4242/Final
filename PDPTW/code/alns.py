...
class ALNS:
    ...
    def __init__(self, problem, nDestroyOps, nRepairOps, nIterations, minSizeNBH, maxPercentageNHB, tau, coolingRate, decayParameter, noise):
        ...
        self.wRepair = [1, 10, 1]  # weights of the repair operators
        ...
        # === 导师新增：全局自适应噪音历史账本 ===
        self.noise_successful_count = 1
        self.no_noise_successful_count = 1
        self.active_noise_label_this_turn = False
    ...
    def execute(self):
        ...
        for i in range(self.nIterations):
            ...
            # 🧹 改为每 500 轮打印一次进度，或者完全注释掉
            if i % 500 == 0:
                print(f'Iteration number {i}')

            # === 进阶：智能早停机制 ===
            if i - self.optimal_iteration_number > 1000:
                print(f"\n🚀 触发智能早停：连续 1000 轮未打破纪录，已收敛至极值，提前结束运算！")
                break
        ...
        print("\n=== 车辆与路线详细报告 ===")
        self.bestSolution.print()  # 🌟 召唤solution自带的完整打印功能
    ...
    def checkIfAcceptNewSol(self):
        ...
        # === 1. 控噪拦截：动态计算本轮用不用噪音 ===
        summation_noise = self.noise_successful_count + self.no_noise_successful_count
        prob_no_noise = self.no_noise_successful_count / summation_noise

        self.active_noise_label_this_turn = self.randomGen.random() > prob_no_noise
        current_turn_noise = self.noise if self.active_noise_label_this_turn else 0.0

        self.tempSolution.computeDistanceWithNoise(self.max_arc_length, current_turn_noise, self.randomGen)
        ...
        if self.tempSolution.distance < self.currentSolution.distance:
            ...
            if self.tempSolution.distance < self.bestDistance:  # update best solution
                ...
                # 【第1处记功点】：成功打破世界纪录
                if self.active_noise_label_this_turn:
                    self.noise_successful_count += 1
                else:
                    self.no_noise_successful_count += 1
    ...
    def destroyAndRepair(self, destroyHeuristicNr, repairHeuristicNr, sizeNBH):
        ...
        if destroyHeuristicNr == 0:
            ...
            # 激活第 9 号算子 Shaw Removal
        elif destroyHeuristicNr == 9:
            destroySolution.executeShawRemoval(sizeNBH, self.randomGen)
